...
// Define a struct which contain a pointer leads to the second struct & a
// place to the string which will put
struct nodeBus
{
    int id;
    char date[10];
...
struct nodeBus *
MakeEmpty(struct nodeBus *L)            // to ensure that the Linked List L is empty
{
...
loadBusFile(struct nodeBus *L)
{
    ...
    while (endLine != NULL) {
        ...
        node = (struct nodeBus *) malloc(sizeof(struct nodeBus));
        if (temp != NULL) {
            struct nodeBus *node;
...
int
isEmptyB(struct nodeBus *L)
{
}

void
printBlist(struct nodeBus *L)
{
...
//the menu or list.
        ...
        scanf("%d", &choice);
        printf("----------------------------------------------------\n");
/*The user has a list and he has to choose a number from 1 to 9 and the
compiler will do what the user requests and call the function for each cases*/
        switch (choice) {
        case 1:
            loadBusFile(headB);
            printBlist(headB);
